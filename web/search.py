        ...
        if result_file.exists():
            ...
            for g in options["gene"]:
                ...
                if not result.empty:
                    ...
                    row_list = result.iloc[0].replace({pd.NA: None, np.nan: None}).to_dict()
                    ...
                    results.append({"success": True, "type": "coregulator_network", "data": row_list,
                                   "title": f"Coregulators of gene {g} (derived from {d} dataset)", "target":g })
                else:
                    print("Row not found")
                    results.append(
                        {"success": False, "message": f"Gene {g} not found in dataset {d}", "target":g})
        else:
            results.append(
